...
if [ "$1" = "all" ]; then
    ...
    if [ -f "$filename" ]; then
        ...
        modnum=$(($(find ./test/ -name "*.py" | wc -l)-1 ))
    ...
    if [ ! -f "$filename" ]; then
        touch $filename
        readarray  -t modulearray  < <(find ./test/ -name "*.py")
        for i in ${!modulearray[@]}; do 
            modulearray[i]=$(echo ${modulearray[i]} | sed 's/..test.//g' | sed 's/.py//'); 
        done
    ...
    for linenum in ${!modnametime[@]}; do
        ...
        readarray  -t modulefiles  < <(find -name "$modulename.py" | /bin/grep -v test)
...
#            echo ${modulename}
            ...
            testfimodificationtime=$(stat -c %Y ./test/$modulename.py)
#            echo $modulename
#            echo $moduletesttime
...
#            echo "------------------------"
            if  [ "$modulemodificationtime" -gt "$moduletesttime" ] ||\
                ...
                python3 -m unittest -v -f test.${modulename}
